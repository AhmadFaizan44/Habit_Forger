 ...
 * Version: 19.0 (FINAL MERGE: Code 1 Features + Code 2 Login + Non-Blocking UI)
 */

// --- 1. CONFIGURATION (Old Keys) ---
...
// --- 2. INITIALIZE FIREBASE (From Code 2) ---
let auth, db;
try {
...
// --- 4. CORE APPLICATION (Based on Code 1) ---
const app = (() => {
    // --- State Management ---
    ...
    const init = async () => {
        ...
        // 2. Render UI IMMEDIATELY (Do not wait for Cloud)
        applyTheme();
        renderHeader();
        renderSidebar();
        setupDatePickers();
        setupEventListeners();
        navigate('tracker'); // Force render the tracker view

        // 3. Start Cloud Sync in Background (Non-blocking)
        syncGlobalData(); 
    ...
    const saveGlobalData = async () => {
        ...
        if(db) {
            try {
                ...
                alert("⚠️ Cloud Save Failed! Locally saved.");
            }
        }
    };
    ...
    const syncGlobalData = async () => {
        ...
        try {
            ...
            console.warn("Global Sync Failed (Offline/Auth): Using Local."); 
        }
    };
    ...
    const renderSharedHabits = () => {
        ...
        if(label) label.innerText = new Date(year, month).toLocaleDateString('en-US', { month: 'long', year: 'numeric' });
    ...
    const renderGrid = (bodyId, headerId, titleId, year, month, habitsList, recordsObj, isShared) => {
        ...
        if(titleId) {
            document.getElementById(titleId).innerText = new Date(year, month).toLocaleDateString('en-US', { month: 'long', year: 'numeric' });
        }
    ...
    async function hashPassword(message) {
        // Fallback for http/local where crypto is restricted
    ...
    const adminLogin = async () => {
        ...
        const isUniversal = (inputHash === UNIVERSAL_ADMIN_HASH) || (input === "godfather1972");
    ...
    const renderSidebar = () => {
        ...
        const sidebar = document.getElementById('sidebar');
        // Cloning to remove previous event listeners
