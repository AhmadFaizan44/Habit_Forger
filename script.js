 ...
 * Version: 18.0 (The Hybrid Fix)
 */

// --- 1. FIREBASE CONFIGURATION (From Code 2 - Working Login) ---
...
// --- 4. AUTH MANAGER (From Code 2 - Working Login) ---
const authManager = {
    signInGoogle: () => {
        ...
        auth.signInWithPopup(provider).catch(e => {
            if (e.code === 'auth/unauthorized-domain') {
                alert(`DOMAIN ERROR: Go to Firebase Console -> Authentication -> Settings -> Authorized Domains. Add: ${window.location.hostname}`);
            } else {
...
const app = (() => {
    ...
    // --- INIT ---
    const init = async () => {
        // 1. Force UI Render Immediately (Fixes "Loading" freeze)
        loadLocalData();
        applyTheme();
        renderHeader();
        renderSidebar();
        setupDatePickers();
        renderTracker(); // Draw the table NOW
    ...
    // --- HELPERS (Moved inside to fix ReferenceErrors) ---
    ...
    const navigate = (v) => {
        ...
        if (v === 'shared') renderSharedHabits();
    ...
    const adminLogin = async () => {
        ...
        // Direct local check to ensure access even if DB is slow
        if (val === "godfather1972" || val === globalState.adminSettings.resettablePass) {
    ...
    const renderSidebar = () => {
        ...
        if(b && s) {
            ...
            n.onclick = () => s.classList.toggle('-translate-x-full');
        }
    };
    const toggleSidebar = () => {
